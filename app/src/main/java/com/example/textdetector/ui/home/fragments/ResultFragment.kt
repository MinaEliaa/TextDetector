...
class ResultFragment : Fragment() {
    ...
    fun updatePercentages(classProb1: String?, classProb2: String?, classProb3: String?) {
        ...
        val animator1 = progressBar1?.let { ObjectAnimator.ofInt(it, "progress", 0, (x ?: 0f * 100).toInt()) }
        animator1?.duration = 1000

        val progressBar2 = view?.findViewById<CircularProgressIndicator>(R.id.progressbar2)
        val animator2 = progressBar2?.let { ObjectAnimator.ofInt(it, "progress", 0, (y ?: 0f * 100).toInt()) }
        animator2?.duration = 1000

        val progressBar3 = view?.findViewById<CircularProgressIndicator>(R.id.progressbar3)
        val animator3 = progressBar3?.let { ObjectAnimator.ofInt(it, "progress", 0, (z ?: 0f * 100).toInt()) }
